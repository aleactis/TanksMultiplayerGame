...
public class SmoothCameraFollow : MonoBehaviour
{
    ...
    LayerMask lineOfSigthMask = 0;
    Vector3 headOffset = Vector3.zero;
    Vector3 centerOffeset = Vector3.zero;
    ...
    {
        Vector3 targetCenter = target.position + centerOffeset;
    ...
    {
        RaycastHit hit;
        if (Physics.Linecast(target, newPosition, out hit, lineOfSigthMask.value))
        {
            velocity = Vector3.zero;
            return hit.point;
        }
    ...
    void SetUpRotation(Vector3 centerPos, Vector3 headPos)
    {
        ...
        //Gerar rotação somente em torno do eixo y
        ...
        //Calcular a posição do centro projetado e a posição do topo no world space
        ...
        float heithToAngle = centerToTopAngle / (centerRayPos.y - topRayPos.y);

        float extraLookAngle = heithToAngle * (centerRayPos.y - centerPos.y);
